...
local function testFunc()
  ...
  -- Preset payload round-trip
  local presetPayload = seam.buildPresetPayload()
  T.assertNotNil(presetPayload, "preset payload built")
  T.assertEquals(seam.PROTO_VERSION, presetPayload.v, "preset payload.v")
  T.assertEquals("preset", presetPayload.t, "preset payload.t")
  T.assertTrue(type(presetPayload.uid) == "string", "preset payload.uid is string")
  T.assertTrue(type(presetPayload.data) == "string" and #presetPayload.data > 0,
    "preset payload.data is non-empty string")

  local presetEncoded = seam.encode(presetPayload)
  T.assertNotNil(presetEncoded, "preset encoded")
  local presetDecoded = seam.decode(presetEncoded)
  T.assertNotNil(presetDecoded, "preset decoded")
  T.assertEquals("preset", presetDecoded.t, "preset round-trip t")
  T.assertEquals(presetPayload.uid, presetDecoded.uid, "preset round-trip uid")
  T.assertEquals(presetPayload.data, presetDecoded.data, "preset round-trip data")
