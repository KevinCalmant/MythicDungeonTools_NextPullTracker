...
-- Content fingerprint that ignores UID and display name, so the same route
-- imported under a different name or with a regenerated UID still matches.
local function presetFingerprint(preset)
  if not (preset and preset.value and MDT and MDT.TableToString) then return nil end
  return MDT:TableToString({ value = preset.value }, false, 1)
end

local function findEquivalentPreset(incomingPreset)
  if not (MDT and MDT.GetDB) then return nil, nil end
  local mdtDB = MDT:GetDB()
  if not (mdtDB and mdtDB.presets) then return nil, nil end
  local dungeonIdx = incomingPreset.value and incomingPreset.value.currentDungeonIdx
  if not dungeonIdx then return nil, nil end
  local presets = mdtDB.presets[dungeonIdx]
  if type(presets) ~= "table" then return nil, nil end

  local incomingFp = presetFingerprint(incomingPreset)
  if not incomingFp then return nil, nil end

  for i, existing in ipairs(presets) do
    if type(existing) == "table" and presetFingerprint(existing) == incomingFp then
      return dungeonIdx, i
    end
  end
  return nil, nil
...
local function selectMDTPreset(dungeonIdx, presetIdx)
  if not (MDT and MDT.GetDB) then return end
  local mdtDB = MDT:GetDB()
  if not mdtDB then return end
  if MDT.UpdateToDungeon then pcall(MDT.UpdateToDungeon, MDT, dungeonIdx, true) end
  mdtDB.currentPreset = mdtDB.currentPreset or {}
  mdtDB.currentPreset[dungeonIdx] = presetIdx
  if MDT.UpdatePresetDropDown then pcall(MDT.UpdatePresetDropDown, MDT) end
  if MDT.UpdateMap then pcall(MDT.UpdateMap, MDT) end
end
...
local function handlePresetMessage(senderShort, payload)
  ...
  -- Dedup: if the same route already exists locally, just switch to it instead
  -- of re-importing. MDT's ImportPreset only dedups by UID, and we regenerate
  -- the UID on every send, so without this check we'd duplicate the preset
  -- in the user's MDT preset list every key.
  local dungeonIdx, existingIdx = findEquivalentPreset(preset)
  if existingIdx then
    selectMDTPreset(dungeonIdx, existingIdx)
    print(CHAT_PREFIX..": switched to matching preset from "..senderShort..".")
  else
    preset.uid = nil -- let MDT generate a fresh UID for the new preset
    MDT:ImportPreset(preset)
    print(CHAT_PREFIX..": auto-imported route from "..senderShort..".")
  end

  -- Restart NPT so it rebuilds its state from the now-current preset.
